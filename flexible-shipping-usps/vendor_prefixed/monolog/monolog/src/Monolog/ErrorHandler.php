 ...
 */
namespace FlexibleShippingUspsVendor\Monolog;
...
class ErrorHandler
{
    ...
    /** @var LogLevel::* */
    private $fatalLevel = \FlexibleShippingUspsVendor\Psr\Log\LogLevel::ALERT;
    /** @var ?string */
     ...
     * @return ErrorHandler
     */
    public static function register(\FlexibleShippingUspsVendor\Psr\Log\LoggerInterface $logger, $errorLevelMap = [], $exceptionLevelMap = [], $fatalLevel = null) : self
    {
    ...
    public function registerFatalHandler($level = null, int $reservedMemorySize = 20) : self
    {
        ...
        $this->fatalLevel = null === $level ? \FlexibleShippingUspsVendor\Psr\Log\LogLevel::ALERT : $level;
    ...
    {
        return ['ParseError' => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::CRITICAL, 'Throwable' => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::ERROR];
    }
    /**
    ...
    {
        return [\E_ERROR => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::CRITICAL, \E_WARNING => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::WARNING, \E_PARSE => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::ALERT, \E_NOTICE => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::NOTICE, \E_CORE_ERROR => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::CRITICAL, \E_CORE_WARNING => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::WARNING, \E_COMPILE_ERROR => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::ALERT, \E_COMPILE_WARNING => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::WARNING, \E_USER_ERROR => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::ERROR, \E_USER_WARNING => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::WARNING, \E_USER_NOTICE => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::NOTICE, \E_STRICT => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::NOTICE, \E_RECOVERABLE_ERROR => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::ERROR, \E_DEPRECATED => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::NOTICE, \E_USER_DEPRECATED => \FlexibleShippingUspsVendor\Psr\Log\LogLevel::NOTICE];
    }
    /**
    ...
    {
        $level = \FlexibleShippingUspsVendor\Psr\Log\LogLevel::ERROR;
        ...
        $this->logger->log($level, \sprintf('Uncaught Exception %s: "%s" at %s line %s', \FlexibleShippingUspsVendor\Monolog\Utils::getClass($e), $e->getMessage(), $e->getFile(), $e->getLine()), ['exception' => $e]);
    ...
    public function handleError(int $code, string $message, string $file = '', int $line = 0, ?array $context = []) : bool
    {
        ...
        if (!$this->hasFatalErrorHandler || !\in_array($code, self::$fatalErrors, \true)) {
            $level = $this->errorLevelMap[$code] ?? \FlexibleShippingUspsVendor\Psr\Log\LogLevel::CRITICAL;
    ...
    public function handleFatalError() : void
    {
        ...
        if ($lastError && \in_array($lastError['type'], self::$fatalErrors, \true)) {
            ...
            if ($this->logger instanceof \FlexibleShippingUspsVendor\Monolog\Logger) {
