...
use FlexibleShippingUspsVendor\Monolog\Formatter\JsonFormatter;
use FlexibleShippingUspsVendor\PhpAmqpLib\Message\AMQPMessage;
use FlexibleShippingUspsVendor\PhpAmqpLib\Channel\AMQPChannel;
use AMQPExchange;
/**
 * @phpstan-import-type Record from \Monolog\Logger
 */
class AmqpHandler extends \FlexibleShippingUspsVendor\Monolog\Handler\AbstractProcessingHandler
{
    /**
     ...
     */
    public function __construct($exchange, ?string $exchangeName = null, $level = \FlexibleShippingUspsVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if ($exchange instanceof \FlexibleShippingUspsVendor\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    private function createAmqpMessage(string $data) : \FlexibleShippingUspsVendor\PhpAmqpLib\Message\AMQPMessage
    {
        $attributes = ['delivery_mode' => 2, 'content_type' => 'application/json'];
        if ($this->extraAttributes) {
            $attributes = \array_merge($attributes, $this->extraAttributes);
        }
        return new \FlexibleShippingUspsVendor\PhpAmqpLib\Message\AMQPMessage($data, $attributes);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return new \FlexibleShippingUspsVendor\Monolog\Formatter\JsonFormatter(\FlexibleShippingUspsVendor\Monolog\Formatter\JsonFormatter::BATCH_MODE_JSON, \false);
    }
}
