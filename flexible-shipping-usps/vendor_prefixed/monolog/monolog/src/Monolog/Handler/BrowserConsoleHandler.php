 ...
 */
class BrowserConsoleHandler extends \FlexibleShippingUspsVendor\Monolog\Handler\AbstractProcessingHandler
{
    /** @var bool */
    ...
    {
        return new \FlexibleShippingUspsVendor\Monolog\Formatter\LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{font-weight: bold} %message%');
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return [\FlexibleShippingUspsVendor\Monolog\Logger::DEBUG => 'debug', \FlexibleShippingUspsVendor\Monolog\Logger::INFO => 'info', \FlexibleShippingUspsVendor\Monolog\Logger::NOTICE => 'info', \FlexibleShippingUspsVendor\Monolog\Logger::WARNING => 'warn', \FlexibleShippingUspsVendor\Monolog\Logger::ERROR => 'error', \FlexibleShippingUspsVendor\Monolog\Logger::CRITICAL => 'error', \FlexibleShippingUspsVendor\Monolog\Logger::ALERT => 'error', \FlexibleShippingUspsVendor\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \FlexibleShippingUspsVendor\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \FlexibleShippingUspsVendor\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \FlexibleShippingUspsVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**
