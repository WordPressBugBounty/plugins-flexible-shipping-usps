...
use FlexibleShippingUspsVendor\Monolog\Formatter\ChromePHPFormatter;
 ...
 */
class ChromePHPHandler extends \FlexibleShippingUspsVendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return new \FlexibleShippingUspsVendor\Monolog\Formatter\ChromePHPFormatter();
    }
    /**
    ...
    protected function send() : void
    {
        ...
        $json = \FlexibleShippingUspsVendor\Monolog\Utils::jsonEncode(self::$json, \FlexibleShippingUspsVendor\Monolog\Utils::DEFAULT_JSON_FLAGS & ~\JSON_UNESCAPED_UNICODE, \true);
        $data = \base64_encode($json);
        if (\strlen($data) > 3 * 1024) {
            self::$overflowed = \true;
            $record = ['message' => 'Incomplete logs, chrome header size limit reached', 'context' => [], 'level' => \FlexibleShippingUspsVendor\Monolog\Logger::WARNING, 'level_name' => \FlexibleShippingUspsVendor\Monolog\Logger::getLevelName(\FlexibleShippingUspsVendor\Monolog\Logger::WARNING), 'channel' => 'monolog', 'datetime' => new \DateTimeImmutable(), 'extra' => []];
