 ...
 */
class CubeHandler extends \FlexibleShippingUspsVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $url, $level = \FlexibleShippingUspsVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
    ...
    protected function connectUdp() : void
    {
        if (!\extension_loaded('sockets')) {
            throw new \FlexibleShippingUspsVendor\Monolog\Handler\MissingExtensionException('The sockets extension is required to use udp URLs with the CubeHandler');
        }
    ...
    protected function connectHttp() : void
    {
        if (!\extension_loaded('curl')) {
            throw new \FlexibleShippingUspsVendor\Monolog\Handler\MissingExtensionException('The curl extension is required to use http URLs with the CubeHandler');
        }
    ...
    protected function write(array $record) : void
    {
        ...
        if ($this->scheme === 'http') {
            $this->writeHttp(\FlexibleShippingUspsVendor\Monolog\Utils::jsonEncode($data));
        } else {
            $this->writeUdp(\FlexibleShippingUspsVendor\Monolog\Utils::jsonEncode($data));
        }
    }
    ...
    private function writeHttp(string $data) : void
    {
        ...
        \FlexibleShippingUspsVendor\Monolog\Handler\Curl\Util::execute($this->httpConnection, 5, \false);
    }
}
