...
use FlexibleShippingUspsVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use FlexibleShippingUspsVendor\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
 ...
 */
class FingersCrossedHandler extends \FlexibleShippingUspsVendor\Monolog\Handler\Handler implements \FlexibleShippingUspsVendor\Monolog\Handler\ProcessableHandlerInterface, \FlexibleShippingUspsVendor\Monolog\ResettableInterface, \FlexibleShippingUspsVendor\Monolog\Handler\FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = \true, bool $stopBuffering = \true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \FlexibleShippingUspsVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\FlexibleShippingUspsVendor\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof \FlexibleShippingUspsVendor\Monolog\Handler\FingersCrossed\ActivationStrategyInterface) {
            $activationStrategy = new \FlexibleShippingUspsVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \FlexibleShippingUspsVendor\Monolog\Logger::toMonologLevel($passthruLevel);
        }
        if (!$this->handler instanceof \FlexibleShippingUspsVendor\Monolog\Handler\HandlerInterface && !\is_callable($this->handler)) {
    ...
    public function reset()
    {
        ...
        if ($this->getHandler() instanceof \FlexibleShippingUspsVendor\Monolog\ResettableInterface) {
    ...
    {
        if (!$this->handler instanceof \FlexibleShippingUspsVendor\Monolog\Handler\HandlerInterface) {
    ...
    public function setFormatter(\FlexibleShippingUspsVendor\Monolog\Formatter\FormatterInterface $formatter) : \FlexibleShippingUspsVendor\Monolog\Handler\HandlerInterface
    {
        ...
        if ($handler instanceof \FlexibleShippingUspsVendor\Monolog\Handler\FormattableHandlerInterface) {
