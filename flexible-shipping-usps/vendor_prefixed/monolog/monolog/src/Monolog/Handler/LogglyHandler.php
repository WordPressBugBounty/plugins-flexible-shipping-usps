...
use FlexibleShippingUspsVendor\Monolog\Formatter\LogglyFormatter;
 ...
 */
class LogglyHandler extends \FlexibleShippingUspsVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $token, $level = \FlexibleShippingUspsVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('curl')) {
            throw new \FlexibleShippingUspsVendor\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the LogglyHandler');
        }
        $this->token = $token;
    ...
    protected function send(string $data, string $endpoint) : void
    {
        ...
        \FlexibleShippingUspsVendor\Monolog\Handler\Curl\Util::execute($ch, 5, \false);
    }
    protected function getDefaultFormatter() : \FlexibleShippingUspsVendor\Monolog\Formatter\FormatterInterface
    {
        return new \FlexibleShippingUspsVendor\Monolog\Formatter\LogglyFormatter();
    }
}
