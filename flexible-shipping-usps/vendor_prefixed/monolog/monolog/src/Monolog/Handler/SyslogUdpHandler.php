...
use FlexibleShippingUspsVendor\Monolog\Handler\SyslogUdp\UdpSocket;
 ...
 */
class SyslogUdpHandler extends \FlexibleShippingUspsVendor\Monolog\Handler\AbstractSyslogHandler
{
    const RFC3164 = 0;
    const RFC5424 = 1;
    const RFC5424e = 2;
     ...
     */
    public function __construct(string $host, int $port = 514, $facility = \LOG_USER, $level = \FlexibleShippingUspsVendor\Monolog\Logger::DEBUG, bool $bubble = \true, string $ident = 'php', int $rfc = self::RFC5424)
    {
        if (!\extension_loaded('sockets')) {
            throw new \FlexibleShippingUspsVendor\Monolog\Handler\MissingExtensionException('The sockets extension is required to use the SyslogUdpHandler');
        }
        parent::__construct($facility, $level, $bubble);
        $this->ident = $ident;
        $this->rfc = $rfc;
        $this->socket = new \FlexibleShippingUspsVendor\Monolog\Handler\SyslogUdp\UdpSocket($host, $port);
    }
    ...
    private function splitMessageIntoLines($message) : array
    {
        ...
        if (\false === $lines) {
            ...
            throw new \RuntimeException('Could not preg_split: ' . $pcreErrorCode . ' / ' . \FlexibleShippingUspsVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $lines;
    }
    /**
     ...
     */
    public function setSocket(\FlexibleShippingUspsVendor\Monolog\Handler\SyslogUdp\UdpSocket $socket) : self
    {
        $this->socket = $socket;
        return $this;
    }
