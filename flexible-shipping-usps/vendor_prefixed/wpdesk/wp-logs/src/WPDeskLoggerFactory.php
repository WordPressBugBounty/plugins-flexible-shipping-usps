...
use FlexibleShippingUspsVendor\Monolog\Handler\StreamHandler;
use FlexibleShippingUspsVendor\Psr\Log\LogLevel;
use FlexibleShippingUspsVendor\WPDesk\Logger\WC\WooCommerceCapture;
use FlexibleShippingUspsVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \FlexibleShippingUspsVendor\WPDesk\Logger\BasicLoggerFactory
{
    const DEFAULT_LOGGER_CHANNEL_NAME = 'wpdesk';
    /** @var string Log to file when level is */
    const LEVEL_WPDESK_FILE = \FlexibleShippingUspsVendor\Psr\Log\LogLevel::DEBUG;
    /** @var string Log to wc logger when level is */
    const LEVEL_WC = \FlexibleShippingUspsVendor\Psr\Log\LogLevel::ERROR;
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\FlexibleShippingUspsVendor\Monolog\Registry::hasLogger($name)) {
            \FlexibleShippingUspsVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\FlexibleShippingUspsVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \FlexibleShippingUspsVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \FlexibleShippingUspsVendor\Monolog\Logger($name);
        }
        if (\FlexibleShippingUspsVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \FlexibleShippingUspsVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\FlexibleShippingUspsVendor\Monolog\Logger $logger)
    {
        if (!\defined('FlexibleShippingUspsVendor\\WC_LOG_THRESHOLD')) {
            \define('FlexibleShippingUspsVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \FlexibleShippingUspsVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \FlexibleShippingUspsVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \FlexibleShippingUspsVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \FlexibleShippingUspsVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\FlexibleShippingUspsVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \FlexibleShippingUspsVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \FlexibleShippingUspsVendor\Monolog\Registry::hasLogger($name);
    }
}
