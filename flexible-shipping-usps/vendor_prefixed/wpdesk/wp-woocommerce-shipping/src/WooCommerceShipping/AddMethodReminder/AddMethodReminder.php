 ...
 */
class AddMethodReminder implements \FlexibleShippingUspsVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    public function hooks()
    {
        ...
        (new \FlexibleShippingUspsVendor\WPDesk\WooCommerceShipping\AddMethodReminder\ClickNoticeTracker($this->method_id))->hooks();
        (new \FlexibleShippingUspsVendor\WPDesk\WooCommerceShipping\AddMethodReminder\DeactivationTrackerData($this->method_id))->hooks();
        (new \FlexibleShippingUspsVendor\WPDesk\WooCommerceShipping\AddMethodReminder\TrackerData($this->method_id, $this->tracker_name))->hooks();
    }
    public function display_notice_if_should()
    {
        $display_strategy = new \FlexibleShippingUspsVendor\WPDesk\ShowDecision\GetStrategy([['page' => 'wc-settings', 'tab' => 'shipping', 'section' => $this->method_id]]);
        if ($display_strategy->shouldDisplay() && $this->shipping_method_settings_saved() && !$this->method_added_in_any_zone($this->method_id)) {
            $notice = new \FlexibleShippingUspsVendor\WPDesk\Notice\Notice(\sprintf(\__('%1$sPlease add now the %2$s shipping method within the shipping zone you want it to be available in →%3$s', 'flexible-shipping-usps'), '<a href="' . \admin_url('admin.php?page=wc-settings&tab=shipping&track_click_notice_method=' . \wp_create_nonce($this->method_id)) . '">', $this->method_title, '</a>'), \FlexibleShippingUspsVendor\WPDesk\Notice\Notice::NOTICE_TYPE_WARNING);
        }
    }
