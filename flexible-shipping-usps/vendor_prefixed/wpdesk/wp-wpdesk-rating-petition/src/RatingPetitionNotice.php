 ...
 */
namespace FlexibleShippingUspsVendor\WPDesk\RepositoryRating;

use FlexibleShippingUspsVendor\WPDesk\PluginBuilder\Plugin\Hookable;
/**
 * Can display rating notices based on watcher time time.
 */
class RatingPetitionNotice implements \FlexibleShippingUspsVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
     ...
     */
    public function __construct(\FlexibleShippingUspsVendor\WPDesk\RepositoryRating\TimeWatcher $method_watcher, $namespace, $plugin_name, $rate_url = '')
    {
    ...
    {
        new \FlexibleShippingUspsVendor\WPDesk\Notice\PermanentDismissibleNotice($this->get_notice_content(), $notice_name, \FlexibleShippingUspsVendor\WPDesk\Notice\Notice::NOTICE_TYPE_INFO);
    }
    /**
     * Get notice content.
     *
    ...
    private function get_notice_content()
    {
        ...
        $content = \sprintf(\__('Awesome, you\'ve been using %s for more than 2 weeks. Could you please do me a BIG favor and give it a 5-star rating on WordPress? ~ Peter', 'flexible-shipping-usps'), $this->plugin_name);
        $content .= '<br/>';
