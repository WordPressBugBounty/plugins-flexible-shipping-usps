...
class OptInPage implements \FlexibleShippingUspsVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    public function wpdesk_tracker_page()
    {
        ...
        $shop = new \FlexibleShippingUspsVendor\WPDesk\Tracker\Shop($shop_url);
        $terms_url = $shop->get_usage_tracking_page();
        $logo = $shop->get_shop_logo_file();
        $logo_url = \plugin_dir_url(__FILE__) . '../../../assets/images/' . $logo;
        $renderer = new \FlexibleShippingUspsVendor\WPDesk\View\Renderer\SimplePhpRenderer(new \FlexibleShippingUspsVendor\WPDesk\View\Resolver\DirResolver(__DIR__ . '/views'));
        // WPCS: XSS ok.
