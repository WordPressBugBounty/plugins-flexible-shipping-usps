...
if (empty($notice_content)) {
    ?>
            <?php 
    \printf(\esc_html__('Hey %s,', 'flexible-shipping-usps'), $username);
    ?><br/>
            <?php 
    \esc_html_e('We need your help to improve <strong>WP Desk plugins</strong>, so they are more useful for you and the rest of <strong>30,000+ users</strong>. By collecting data on how you use our plugins, you will help us a lot. We will not collect any sensitive data, so you can feel safe.', 'flexible-shipping-usps');
    ?>
            <a href="<?php 
...
\esc_html_e('Allow', 'flexible-shipping-usps');
?></button>
    </p>
</div>
...
?>',
            {
                ...
                data: {
                    security: '<?php 
echo \esc_attr(\wp_create_nonce(\FlexibleShippingUspsVendor\WPDesk_Tracker::WPDESK_TRACKER_NOTICE));
?>',
